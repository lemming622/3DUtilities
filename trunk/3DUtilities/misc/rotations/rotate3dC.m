...
%
%   Modifed to update figure continously/smoothly
...
if(isempty(rotaObj))
   ...
   if(VIEWBOX) set(rdata.outlineObj,'Visible','on'); end
   set(gcbf,'WindowButtonMotionFcn','rotate3dC(''motion'')');
end
...
rotaObj = findobj(allchild(gcbf),'Tag','rotaObj');
if isempty(rotaObj) | ...
    ~strcmp(get(gcbf,'WindowButtonMotionFcn'),'rotate3dC(''motion'')')
   return;
else
...
set(fig,'WindowButtonDownFcn','rotate3dC(''down'')');
set(fig,'WindowButtonUpFcn'  ,'rotate3dC(''up'')');
