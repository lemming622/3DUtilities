package edu.ncsa.model.graphics.jogl2;
...
public class ModelViewer extends AbstractModelViewer implements Runnable, GLEventListener, KeyListener, MouseListener, MouseMotionListener, MouseWheelListener, ActionListener
{
   ...
   */
  private void saveImage(GL2 gl, String filename)
  {
    BufferedImage image = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
    int[] rgb = grabImage(gl, GL2.GL_BACK); 
   ...
   */
  private void saveVisualHullView(GL2 gl, String path)
  {
    BufferedImage image = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
    int[] pixels_rgb = null;
    ...
    pixels_rgb = grabImage(gl, GL2.GL_BACK);    
    ...
    if(SAVE_AXIS){   //Draw reference axis and capture image
      ...
      pixels_axis = grabImage(gl, GL2.GL_BACK);
  ...
  {    
	 ...
	 */
	private int[] grabImage(GL2 gl, int gl_color_buffer)
	{
  ...
  public void init(GLAutoDrawable drawable)
  {
    ...
    gl.glLightfv(GL2.GL_LIGHT0, GL2.GL_DIFFUSE, light_diff, 0);
    gl.glEnable(GL2.GL_LIGHT0);
    
    gl.glLightfv(GL2.GL_LIGHT1, GL2.GL_POSITION, light1_position, 0);
    gl.glLightfv(GL2.GL_LIGHT1, GL2.GL_DIFFUSE, light_diff, 0);
  ...
  public synchronized void display(GLAutoDrawable drawable)
  {
    ...
    if(OUTLINE || HIGHLIGHTS || ILLUSTRATION || METAL || RAYTRACE || SELECTING || BENDING_JOINT || SAVE_VISUAL_HULL_VIEW || SAVE_DEPTH || SAVE_POINTS_CAMERA){
      gl.glGetDoublev(GL2.GL_TRANSPOSE_PROJECTION_MATRIX, projection_tmp, 0);
      gl.glGetDoublev(GL2.GL_TRANSPOSE_MODELVIEW_MATRIX, modelview_tmp, 0);
    ...
    if(GRAB_IMAGE){
    	grabbed_image = grabImage(gl, GL2.GL_BACK);
      GRAB_IMAGE = false;	
    }
  ...
  private void refreshList()
  {
	 ...
	 */
	private void drawSphere(GL2 gl, float x, float y, float z, float sx, float sy, float sz)
	{
	  GLU glu = new GLU();
	 ...
	 */
	private void drawAxis(GL2 gl, float scale, int line_width)
	{
	 ...
	 * @param line_width the width of the lines when rendering the axis
	 */
	private void drawCalibrationAxis(GL2 gl, float scale, int line_width)
	{
	  gl.glDisable(GL2.GL_LIGHTING);
	  gl.glPushMatrix();
   ...
   */
  private void bindTextures(GL2 gl, Mesh mesh, boolean FORCE)
  {
   ...
   */
  private void drawPCs(GL2 gl, Mesh mesh, float scl)
  {
   ...
   * @param mesh the mesh to draw
   */
  private void drawPoints(GL2 gl, Mesh mesh)
  {
   ...
   * @param mesh the mesh to draw
   */
  private void drawEdges(GL2 gl, Mesh mesh)
  {
   ...
   */
  private void drawOutline(GL2 gl, Mesh mesh, double[][] M)
  {
  	Vector<Point> vertices = mesh.getVertices();
   ...
   * @param mesh the mesh to draw
   */
  private void drawSolid(GL2 gl, Mesh mesh)
  {
  	Vector<Face> faces = mesh.getFaces();
  	Vector<Point> vertices = mesh.getVertices();
   ...
   */
  private void drawShaded(GL2 gl, Mesh mesh, boolean SMOOTH)
  {
   ...
   * @param mesh the mesh to draw
   */
  private void drawShadedFlat(GL2 gl, Mesh mesh)
  {
  	Vector<Point> vertices = mesh.getVertices();
  	Vector<Color> vertex_colors = mesh.getVertexColors();
  	Vector<Face> faces = mesh.getFaces();
    ...
    if(lighting == DrawOption.ENABLED){
    	...
    	gl.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT, new float[]{0.2f, 0.2f, 0.2f, 1.0f}, 0);
    	gl.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_DIFFUSE, new float[]{0.8f, 0.8f, 0.8f, 1.0f}, 0);
   ...
   */
  private void drawShadedFlat(GL2 gl, Mesh mesh, Vector<Integer> selected_faces)
  {
  	Vector<Point> vertices = mesh.getVertices();
  	Vector<Face> faces = mesh.getFaces();
    Point norm;
   ...
   * @param mesh the mesh to draw
   */
  private void drawShadedSmooth(GL2 gl, Mesh mesh)
  {
  	Vector<Point> vertices = mesh.getVertices();
   ...
   * @param mesh the mesh to draw
   */
  private void drawShadedDegenerate(GL2 gl, Mesh mesh)
  {
  	Vector<Point> vertices = mesh.getVertices();
  	Vector<Color> vertex_colors = mesh.getVertexColors();
  	Vector<Face> faces = mesh.getFaces();
   ...
   */
  private void drawHighlights(GL2 gl, Mesh mesh, double[][] M)
  {
  	Vector<Point> vertices = mesh.getVertices();
  	Vector<Point> vertex_normals = mesh.getVertexNormals();
  	Vector<Face> faces = mesh.getFaces();
   ...
   * @param M the current modelview matrix
   */
  private void drawIllustration(GL2 gl, Mesh mesh, double[][] M)
  {
  	Vector<Point> vertices = mesh.getVertices();
  	Vector<Point> vertex_normals = mesh.getVertexNormals();
  	Vector<Face> faces = mesh.getFaces();
   ...
   * @param M the current modelview matrix
   */
  private void drawMetal(GL2 gl, Mesh mesh, double[][] M)
  {
  	Vector<Point> vertices = mesh.getVertices();
  	Vector<Face> faces = mesh.getFaces();
